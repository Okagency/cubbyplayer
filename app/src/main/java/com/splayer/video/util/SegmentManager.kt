    ...
    private val segmentFile: File
        get() {
            ...
            return if (customPath != null) {
                File(customPath, "cubbyplayer.per")
            } else {
                // 기본값: 앱 외부 저장소
                File(context.getExternalFilesDir(null), "cubbyplayer.per")
            }
        }
        ...
        return when (pathType) {
            PATH_TYPE_SUBTITLES -> {
                ...
                try {
                    ...
                    // 쓰기 테스트
                    val testFile = File(subtitlesDir, "cubbyplayer.per")
                    if (!testFile.exists()) {
            ...
            PATH_TYPE_APP_CACHE -> {
                File(context.cacheDir, "cubbyplayer.per")
            }
            else -> File(context.getExternalFilesDir(null), "cubbyplayer.per")
        }
    }
